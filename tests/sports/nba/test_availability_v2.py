...
def con() -> sqlite3.Connection:
    ...
    # G_dark: no inactive rows at all -> unknown, never 0.0.
...
def test_no_inactive_rows_yields_none_never_zero(con) -> None:
    ...
    for k, v in dark.items():
        assert v is None, f"{k} scored {v!r} for a game with no published list - must be None, never 0.0"
...
def test_authoritative_membership_is_game_level(con) -> None:
    assert AV.games_with_authoritative_inactives(con) == {"G_both", "G_home_ok", "G_away_ok"}
